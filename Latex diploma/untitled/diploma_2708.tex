...
Pomemben pojem pri iskanju teh vektorjev je numerični zaklad. %Predstavljene in dokazane so njegove uporabne lastnosti, kot je npr. konveksnost.
...
Naš algoritem primerjamo z drugima algoritmoma na nekaj primerih in ugotovimo, da deluje najhitreje, če najde rešitev samo z lastnimi vektorji matrike $K$. %, drugače je algoritem CPU hitrejši, algoritem Cardna pa je hitrejši samo za manjše matrike.
...
An important concept when searching for these vectors is the field of values (or numerical range). %Some of its useful properties are presented and proved, such as convexity.
...
Further, we compare our algorithm with the algorithms from the other two papers on several examples and we conclude that our algorithm is faster if we can find a solution with only one eigenanalysis of matrix $K$. %, otherwise CPU's algorithm is faster and Carden's algorithm is faster only for smaller matrices.
...
\paragraph{Algoritem:}
\begin{enumerate}[1.]
\item Poiščemo zunanjo aproksimacijo $W(A)$ (pravokotnik, katerega stranice so vzporedne z realno in imaginarno osjo), tako da izračunamo najbolj levo in najbolj desno lastno vrednost $H_\theta =(e^{i\theta}A+e^{-i\theta}A^\ast)/2$ za $\theta =0, \pi/2$. %S tem dobimo zunanjo aproksimacijo zaloge vrednosti, ki seka $\partial W(A)$ v robnih točkah. %RAZLAGA Zunanja aproksimacija bo pravokotnik, katerega stranice so vzporedne z realno in imaginarno osjo. 
...
\item Nadaljujemo s konstrukcijo notranje aproksimacije $W(A)$ (štirikotnik z og\-liš\-či v robnih točkah $\partial W(A)$) z uporabo lastnih vektorjev najbolj leve in desne lastne vrednosti $H_{\theta}$.% RAZLAGA Notranja aproksimacija bo štirikotnik z oglišči v robnih točkah $\partial W(A)$.
...
Za prvi primer vzemimo naključno generirano $10\times 10$ realno matriko $A$ in $\mu = 1.0419$, ki je znotraj numeričnega zaklada.%, slika \ref{fig:p1}.
$$\verb+A = randn(10)+$$
...
[b, napaka] = funkcija(1i*A,XX,LV1,n);
