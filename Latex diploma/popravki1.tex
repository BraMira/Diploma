...
Če pa $\mu$ ni lastna vrednost matrike $A$, je $A-\mu I$ nesingularna in moramo izotropni vektor izračunati. Tudi če matrika $A$ ni realna imamo opravka s kompleksno matriko, ko je $\mu$ kompleksno število.\\
...
\item \label{l7} Projekcija: $W(H)= \Re( W(A))$, za $\forall A\in \C^{n\times n}$, kjer s $H$ označimo hermitsko matriko.
...
\item
\begin{itemize}
\item Konveksnost: bomo dokazali v naslednjem poglavju.
...
\item Zaradi unitarne transformacije postane površina enotske Euklidske krogle invariantna in so kompleksna števila, ki sestavljajo množici $W(U^\ast AU)$ in $W(A)$ enaka. 
...
\begin{izrek}[\emph{Toeplitz-Hausdorfferjev izrek}]\cite{zaloga}
...
Ker pa je $\mu$ konveksna kombinacija $w$ in $\theta_1$, lahko tudi njemu določimo izotropni vektor, \ref{fig:triangle}.\\
\begin{figure} [h]
\centering
...
\begin{table}[H]
\caption{A in $\mu = 5000+10000\imath$ kompleksna in algoritem najde rešitev v 1 koraku.}
\begin{tabular}{|l|c|c|c|}
\hline
Algoritem & Čas & Koraki & Napaka\\
\hline
...
\begin{table}[H]
\caption{A kompleksna, $\mu = 12000+10000\imath$ in algoritem najde rešiev v 2 korakih.}
\begin{tabular}{|l|c|c|c|}
\hline
Algoritem & Čas & Koraki & Napaka\\
\hline
